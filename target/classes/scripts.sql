...
--Вывести все логи приложения за последние сутки
SELECT id, level_id, app_id, message, class, thread, date
FROM public."Log" where date::date > (now()::date - '1 day'::interval);

--вывести количество записей определенного уровня (к примеру, error) для конкретного приложения.
...
FROM("Log" as log JOIN "Level" as level ON(log.level_id=level.id) JOIN "App" as app ON (log.app_id=app.id))
where level.name='INFO' and log.date::date > (now()::date - '1 days'::interval) GROUP BY (app) ;

--Написать запрос, который выводит резултатат по всем приложениям и по всем ошибкам:
SELECT app.name as app,level.name as level,count(level)
FROM("Log" as log JOIN "Level" as level ON(log.level_id=level.id) JOIN "App" as app ON (log.app_id=app.id))
GROUP BY (app,level) ;

--удалить все записи определенного приложения, которые старше 4х суток
DELETE FROM "Log" as log
WHERE log.app_id IN (SELECT id FROM "App" as app WHERE app.name = 'app2')
  and log.date::date <(now()::date - '1 days'::interval) ;